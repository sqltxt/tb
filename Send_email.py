...
def send():
    ...
    # 邮件对象:
    msg = MIMEMultipart()
    msg['From'] = _format_addr(global_TB.username +'<%s>' % from_addr)
    msg['To'] = _format_addr(u'管理员 <%s>' % to_addr)
    msg['Subject'] = Header(u'来自'+global_TB.username+'的问候……', 'utf-8').encode()

    # 邮件正文是MIMEText:
    msg.attach(MIMEText('send with file...', 'plain', 'utf-8'))

    # 添加附件就是加上一个MIMEBase，从本地读取一个图片:
    with open(global_TB.path+global_TB.fbk, 'rb') as f:
        # 设置附件的MIME和文件名，这里是png类型:
        mime = MIMEBase('text', 'fbk', filename=global_TB.fbk)
        # 加上必要的头信息:
        mime.add_header('Content-Disposition', 'attachment', filename=global_TB.fbk)
